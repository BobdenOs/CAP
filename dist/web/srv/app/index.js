...
`
      ...
      if (isOffline) {
        ...
        for (const file of files) {
          ...
          cache.put(`https://${rootDomain.replace('offline.', '')}/apps/${application}/${file}`, new Response(await cds.utils.fs.promises.readFile(`${appFolder}/app/${file}`), { headers: { 'content-type': mimes[ext] || 'plain/text' } }))
        }
      }
    } finally {
      cds.db = rootDB
